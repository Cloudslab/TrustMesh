...
class ResponseManager:
    ...
    @staticmethod
    def _generate_keys():
        keys_dir = os.path.join(os.getcwd(), 'compute_keys')
        os.makedirs(keys_dir, exist_ok=True)
        client_public_file, client_secret_file = zmq.auth.create_certificates(keys_dir, "compute_client")
        client_public, client_secret = zmq.auth.load_certificate(client_secret_file)
        return client_public, client_secret

    def connect(self):
        if self.is_connected:
            print("compute Node is already connected.")
            return
        ...
        print(f"compute Node connected to IoT device at {self.iot_address}")

    def disconnect(self):
        if not self.is_connected:
            print("compute Node is not connected.")
            return

        self.socket.close()
        self.context.term()
        self.is_connected = False
        print("compute Node disconnected.")

    def send_message(self, message):
        if not self.is_connected:
            print("compute Node is not connected. Call connect() first.")
            return
    ...
    def public_key(self):
        """
        Get the public key of the compute node.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: python response_manager.py <iot_ip> <iot_port> <iot_public_key>")
        sys.exit(1)
    ...
    compute_node = ResponseManager(iot_address, iot_public_key)
    compute_node.connect()
    ...
    try:
        while True:
            ...
            if message.lower() == 'quit':
                break
            compute_node.send_message(message)
    finally:
        compute_node.disconnect()
